...
If the user didn't provide the nessesary details. A good idea is to include an error message letting them know for example they are missing a element. Use the interactive example below to test.

<SignInForm />

## Using types on input fields
The type field helps to give browser a undestanding if the element you are trying.